...
class Wx_housecenter extends CI_Controller{
    ...
    public function index(){
        ...
        $this->load->view('wechat/wx_housecenter.php',$arr);
    }
    public function find_house(){

        $city = $_POST['Sinput2'];
        $street = $_POST['Sinput1'];
...
//                var_dump($soutitle);
        ...
        $soustreet=$this->house_zhong_model->get_sou_street($all);
        ...
        $arr =array(
            ...
            'hs_city'=>$soucity,
            'hs_des'=> $soudes,
            'sou_street'=>$soustreet
        );
        $arr=array_merge($arr,$fenye);
        if($all==null){
            redirect('wx_housecenter');
        }else{
            $this->load->view('wechat/wx_housecenter.php',$arr);
        }
    }

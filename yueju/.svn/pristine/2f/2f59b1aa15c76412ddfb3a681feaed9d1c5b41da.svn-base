...
class Room extends CI_Controller{
    ...
    public function do_collect()//ajax传入收藏
    {
        $user_id=$this->input->get('user_id');
        $house_id=$this->input->get('house_id');
        $this->room_model->do_collect($user_id,$house_id);
    }

    public function do_del_collect()
    {
        $user_id=$this->input->get('user_id');
        $house_id=$this->input->get('house_id');
        $this->room_model->do_del_collect($user_id,$house_id);
    }
